...
class MovieListDataSourceTests: XCTestCase {
    ...
    func testResultSuccessDataSource(page: Int, success succeed: (@escaping (Bool, MovieListModel?) -> Void)) {
        let movieListDS = MovieListDataSource()
        movieListDS.getResponse(page: page) { (result) in
            succeed(true, result)
        } failure: {
            succeed(false, nil)
        }
    ...
    func testGetResponseIteratively() {
        ...
        for index in 0..<iterations {
            ...
            testResultSuccessDataSource(page: page, success: { (result, data) in
                ...
                XCTAssertEqual(data?.total_pages, 500, "returned results not equal to 500")
                XCTAssertEqual(data?.total_results, 10000, "returned results not equal to 10000")
